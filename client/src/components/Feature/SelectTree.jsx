...
function SelectTree({ placeholder, categories, subcategories, value, onChange }) {
  ...
  const handleItemSelectionToggle = (event, itemId, isSelected) => {
    if (isSelected && itemId.startsWith('sub-')) {
        ...
        onChange(parseInt(pureId));  // 把純 ID 傳出去
        handleClose();
    }
  };
        ...
        <Box sx={{ maxHeight: 300, minWidth: 250, overflowY: 'auto', p: 1 }}>
          ...
          >
            {Array.isArray(categories) && categories.map(cat => (
                <TreeItem key={`cat-${cat.id}`} itemId={`cat-${cat.id}`} label={cat.name}>
                    {Array.isArray(subcategories) &&
                    subcategories
                        .filter(sub => sub.parentId === cat.id)
                        .map(sub => (
                        <TreeItem key={`sub-${sub.id}`} itemId={`sub-${sub.id}`} label={sub.name} />
                        ))}
                </TreeItem>
            ))}
          </SimpleTreeView>
