...
function Navbar() {
  ...
  return (
  <nav className="w-full h-20 top-0 right-0 bg-[var(--primary-color)] z-110">
      {/* start cart */}
      ...
      <div className="container-mid h-full grid grid-cols-[1fr_4fr_1fr] justify-center gap-rwd px-2">
        {/* start nav left (logo)*/}
          <Link to={"/"} className="my-2 bg-[var(--quaternary-color)] text-white text-3xl flex justify-center items-center">
           Logo
          </Link>
        {/* end  nav left */}
        ...
        >
            ...
            {ready ? (
              categories.map((parent, i) => (
                ...
                >
                  <Link to={`/product`} state={{ initialLabel: parent.id }}>
                    <motion.p
                      ...
                      animate={{
                        ...
                        color: hoverIndex === parent.id ? "var(--tertiary-color)" : "var(--quaternary-color)",
                      }}
                  ...
                  <AnimatePresence>
                    <motion.div
                      className="navbar-menu-mid bg-[#92886d]/95 h-[70%] aspect-square rotate-45"
                      initial={{ opacity: 0 }}
              ...
              <p className="col-start-3 text-sm text-[var(--quaternary-color)]">分類載入中...</p>
            )}
          </div>
          {/* end parent category */}
          ...
          <motion.div
            className="absolute w-full navbar-items-mid bg-[#92886d]/95 overflow-hidden rounded-2xl"
                ...
                subcategories
                  ...
                  .map((sub, i) => (
                    <Link
                      ...
                      key={i}
                      className="text-center text-[var(--primary-color)] hover:text-white max-lg:text-sm"
                    >
                      {sub.name}
                    </Link>
                  ))}
          ...
          <div className="flex justify-end items-center gap-1 w-full">
            ...
            >
              <IoMdCart
                ...
                className="text-2xl text-[var(--quaternary-color)] hover:text-[#F5C45E] transition-colors duration-200 cursor-pointer"
              />
            </motion.div>
            <div className="h-full w-1/5 flex justify-center items-center">
              {isAuthenticated ? (
                ...
                <Link
                  to={"/sign"}
                  className="text-2xl cursor-pointer text-[var(--quaternary-color)] hover:text-[#91c883] transition-colors duration-200"
                >
                  <IoLogIn />
                </Link>
              )}
